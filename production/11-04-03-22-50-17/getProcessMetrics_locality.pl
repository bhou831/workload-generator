...
my $o_total_map=0;
my $o_local_map=0;
my $total_local_exetime=0;
my $total_nonlocal_exetime=0;
my %nodes;
my $totalLocalityB=0;
my $localityRateNode=0;
...
=cut
	$startTime=1301889021000;
	$stopTime=1301893913;
	if($parameter=~m/^log\.txt/){
...
foreach my $j (@jobs) {
	...
	#print $j;
	my $notneed=0;
	if($j=~m/_che_hadoop-0.20.2-test/){
	    #$jobtime=$1/1000;
	    if($j=~m/job_(\w+)_che/){
		    $jobname=$1;
		}

		open(TEST,"$j")||die "Can not open file!";
        my  @ttarray=<TEST>;
		for(my $jj=0;$jj<@ttarray;$jj++){
			if($ttarray[1]=~m/SUBMIT_TIME=\"(\d+)\"/ or $ttarray[$k]=~m/START_TIME=\"(\d+)\"/ or $ttarray[$k]=~m/LAUNCH_TIME=\"(\d+)\"/){
				$jobtime=$1;
				#print "JOB Time is:".$jobtime."\tStart Time is: $startTime\tStop Time is: $stopTime"."\n";
				if ($jobtime < $startTime or ($jobtime/1000) > $stopTime){
					#print "JOB Time is:".$jobtime."\tStart Time is: $startTime\tStop Time is: $stopTime"."\n";
					$notneed=1;
					close TEST;
					last;
		...
		#print ($jobtime/1000)."\t".$startTime."\t".$stopTime."\n";
			...
			for(my $k=0;$k<@array;$k++){
				...
				my $line=$array[$k];
				#print $line."\n";
			    if ($line=~m/\(Launched\smap\stasks\)\((\d+)\)\]\[\(DATA_LOCAL_MAPS\)\(Data-local\smap\stasks\)\((\d+)\)/) {
					$o_total_map+=$1;
					$o_local_map+=$2;
					#print $2."\n";
			    }
                elsif(!($line=~m/TASK_TYPE=\"SETUP\"/) and !($line=~m/TASK_TYPE=\"CLEANUP\"/) ){
					...
					if ($line=~m/TASKID=\"(\S+)\"/) {
						...
						if($line=~m/TASK_TYPE=\"MAP\"/){
							    ...
							    if ($line=~m/TASK_STATUS=\"SUCCESS\"/) {
									if ($line=~m/HOSTNAME=\"\/(\S+)\/(\S+)\"/) {
										$tasks{$taskid}{"location"}=$2;
										$taskAssign=$2;
										if ($tasks{$taskid}{"datasource"}=~m/$taskAssign/) {
										    $tasks{$taskid}{"local"}=1;
										    $nodes{$taskAssign}{$taskid}=1;
											$localTask++;
											#print "Local".$localTask."\n";
										}
										else{
											$nonlocalTask++;
											#print $nonlocalTask."\n";
											$tasks{$taskid}{"local"}=0;
											$nodes{$taskAssign}{$taskid}=0;
										}
									}
...
#                            }
			...
			$queueTime{$jobname}=($jobStart-$jobtime)/1000;
			$avgQueueTime+=($jobStart-$jobtime)/1000;
			#print $avgQueueTime."\n";
			#print %queueTime."\n";
...
# process the local and non-local process time
foreach my $ttt (%tasks) {
	if (exists($tasks{$ttt}{"local"})) {

	if ($tasks{$ttt}{"local"}==1) {
		$total_local_exetime+=$tasks{$ttt}{"stop"}-$tasks{$ttt}{"start"};
		#print "local:".$ttt.":".($tasks{$ttt}{"stop"}-$tasks{$ttt}{"start"}).":".$tasks{$ttt}{"location"}.":".$tasks{$ttt}{"datasource"}."\n";
	}
	elsif ($tasks{$ttt}{"local"}==0){
		$total_nonlocal_exetime+=$tasks{$ttt}{"stop"}-$tasks{$ttt}{"start"};
		#print "non-local:".$ttt.":".($tasks{$ttt}{"stop"}-$tasks{$ttt}{"start"}).":".$tasks{$ttt}{"location"}.":".$tasks{$ttt}{"datasource"}."\n";
	}
	}
}
open (LOC,">localitydiff.txt")||die "No disk space!";
foreach my $node (sort keys %nodes){
    my $nodelocal=0;
    my $nodenonlocal=0;
    my $counterlocal=0;
    my $counternonlocal=0;
    #print $node."\n";
    foreach my $tid (keys %{$nodes{$node}}){
    	if ($nodes{$node}{$tid}==1){
	    $nodelocal+=$tasks{$tid}{"stop"}-$tasks{$tid}{"start"};
	    $counterlocal+=1;
	}
	elsif($nodes{$node}{$tid}==0){
            $nodenonlocal+=$tasks{$tid}{"stop"}-$tasks{$tid}{"start"};
            $counternonlocal+=1;
        }
    }
    if($counterlocal!=0 and $counternonlocal!=0){
        $localityRateNode+=1;
	$totalLocalityB+=((($nodenonlocal/$counternonlocal)-($nodelocal/$counterlocal))/($nodelocal/$counterlocal));
        #print LOC $node."\t local-benefit:".((($nodenonlocal/$counternonlocal)-($nodelocal/$counterlocal))/($nodelocal/$counterlocal))."\n";
    }
    #if ($counterlocal!=0){
    #    print LOC $node."\t local:".($nodelocal/$counterlocal)."\n";
    #}    
    #if ($counternonlocal!=0){
    #    print LOC $node."\t non-local:".($nodenonlocal/$counternonlocal)."\n";
    #}
}
    print LOC "Total Benefit:".($totalLocalityB/$localityRateNode)."\n";
...
#		print OUT "Reduce-Sort start at:".$stages{$keys}{"shufflestop"}."\t Stop at:".$stages{$keys}{"sortstop"}."\t"."Time: ".(($stages{$keys}{"sortstop"}-$stages{$keys}{"shufflestop"})/1000)."\n";
	...
	print OUT "Locality Rate:\t".$localTask/($localTask+$nonlocalTask)."\n";
	print OUT "LOCAL TASK TIME:\t".$total_local_exetime/$localTask."\n";
	print OUT "NON-LOCAL TASK TIME:\t".$total_nonlocal_exetime/$nonlocalTask."\n";
	if ($o_total_map!=0) {
	   print OUT "Original Locality Rate:\t".$o_local_map/$o_total_map."\n";
    }
	$jobcounter=0;
	...
	$avgExeTime=$avgExeTime/$realJob;
	$avgResponse=$avgResponse/($realJob);
	foreach $keys (sort keys(%queueTime)) {
		#print $keys."\n";
...
#		print OUT "Reduce-Sort start at:".$stages{$keys}{"shufflestop"}."\t Stop at:".$stages{$keys}{"sortstop"}."\t"."Time: ".(($stages{$keys}{"sortstop"}-$stages{$keys}{"shufflestop"})/1000)."\n";
	...
	print OUT "Average Queue Time:\t".($avgQueueTime/1000)."\n";
	print OUT "Average Exe Time:\t".$avgExeTime."\n";
	print OUT "Average Block Exe Time:\t".$avgBlockprocess."\n";
	print OUT "Locality Rate:\t".$localTask/$totalTask."\n";
	if ($o_total_map!=0) {
	   print OUT "Original Locality Rate:\t".$o_local_map/$o_total_map."\n";
    }
	for (my $kkk=0;$kkk<@startTimes ;$kkk++) {
