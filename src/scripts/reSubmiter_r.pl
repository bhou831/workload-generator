...
sub resubmit{
    ...
    foreach $parameter (@ARGV){
	    ...
	    while(<SIM>){
		my $line=$_;
		if($line=~m/hadoop jar/){
	 	    $simulate{$tmp}{"command"}=$line;
		    #print $line."\n";
		}
		elsif($line=~m/hadoop fs/){
		    $simulate{$tmp}{"clean"}=$line;
		}
		elsif($line=~m/date/){
...
while($i<@jobStart){
    ...
    print OUT "\n".$simulate{$jobStart[$i]}{"clean"}."\n";
    close OUT;
    $command="sh $date/simulate$curTime.sh &\n";
    #print "$command\tStart at:$j\n";

    #print OUT $simulate{$jobStart[$i]}{"clean"}."\n";
    ...
    $j+=$sleeptime[$reverse-$i];
    sleep int($sleeptime[$reverse-$i]/1000);
    $i++;
}
